...
public interface PartidaRepository extends JpaRepository<Partida, Integer>{
    ...
    @Query("select p from partidas p where p.clube_da_casa_id=:clubeDaCasa.id order by p.data_partida desc")
    public List<Partida> findByClubeDaCasa(Clube clubeDaCasa);

    @Query("select p from partidas p where p.clube_visitante_id=:clubeVisitante.id order by p.data_partida desc")
